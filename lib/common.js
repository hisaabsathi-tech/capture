  ...
  const H = {
    ...
    onControl(callback) {
      chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
        ...
        // test-only: runs the same finalize path as the real submit button,
        // without touching the courier's Create Shipment / Confirm Booking
        if (msg?.type === 'TEST_SUBMIT') {
          Promise.resolve(callback('TEST_SUBMIT')).then(r => sendResponse({ ok: true, ...(r || {}) }));
          return true;
        }
        return false;
      });
